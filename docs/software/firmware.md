...
- **Accelerometer**: First, since the flight controller is mounted upside down on the frame, go to Config &rarr; Full Parameter List and set `AHRS_ORIENTATION` to `8` (Roll180). Then go to Setup &rarr; Mandatory Hardware &rarr; Accel Calibration. Follow the prompts to place the drone level, on its left, right, nose down, nose up, and on its back.
